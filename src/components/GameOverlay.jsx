...
export default function GameOverlay() {
    const { selectedScene, gameStarted, foundDifferences, differences, returnToScenePicker, backToMemorize } =
        useStore();
            ...
            <div className="pointer-events-none fixed left-1/2 top-20 z-10 w-[min(92vw,28rem)] -translate-x-1/2 rounded-2xl border border-white/25 bg-slate-900/55 px-4 py-3 text-center text-white shadow-lg backdrop-blur-md">
                {!gameStarted ? (
                    <p className="pointer-events-none text-sm leading-relaxed">
                        Arraste com o botão esquerdo para girar a cena; role para aproximar. Memorize os detalhes.
                        Depois use <span className="font-semibold text-amber-300">Iniciar desafio</span> — três
                        objetos somem e você deve clicar onde estavam.
                    </p>
                ) : (
